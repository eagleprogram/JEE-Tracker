...
// Calendar scope (see connectGoogleCalendar() in firebase-sync.js).
//
// Scope of what's implemented: a manual "Connect & Sync" action (the icon
// button in the Holiday Reference card) that (1) pushes the next 7 days of
// planner tasks into the user's Google Calendar as all-day events, tagged
// so re-syncing never creates duplicates, and (2) lists the user's own
// upcoming Google Calendar events underneath the Holiday Reference iframe.
// This is NOT a silent background two-way sync — see the HONEST LIMIT
// comment in firebase-sync.js on why the access token this depends on is
// short-lived; a real always-on background sync would need a server-side
// refresh-token flow (the same shape as the existing FCM push job in
// server/), which is a much bigger project than this button.
...
// Each event carries extendedProperties.private.jeeTrackerTaskId = the
// task's own id, so re-running this later (or after adding new tasks)
// skips anything already pushed instead of creating duplicates.
...
// Renders the user's own next-14-days Google Calendar events into
// #google-calendar-events, underneath the Holiday Reference iframe.
...
// (index.html). One click: connect (or reuse an already-connected
// session), push this week's planner tasks out, pull the user's own
// events back in.
