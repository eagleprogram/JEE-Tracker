...
function buildReportCanvas(days, title) {
    ...
    // REVERTED: a previous pass (a) enlarged these cells (27→30, gap 7→8)
    // and (b) pushed heatmapGridStartY down to vertically center the grid
    // against the pie's height. Both were meant to make the two sections
    // "match," but in practice (a) made the grid too large — pushing the
    // legend/whole section down and risking collision with the table below
    // on a tall (5-row, ~monthly) grid — and (b) created a large, oddly
    // empty gap between the day-of-week labels and the actual colored grid
    // on a short (1-row, weekly) one. Both are reverted back to the
    // original tight layout: title → labels → grid with a fixed 16px gap,
    // same rhythm the pie already has (a fixed 40px title→circle gap) —
    // that fixed-gap consistency IS the alignment, without introducing any
    // dead space or growing past what the pie's own height comfortably
    // allows even in the worst case (5 rows).
...
ctx.textAlign = "left";
    ...
    // BUG FIX: "Que." used to be center-aligned within its column slot
    // (textAlign "center" at questions + half the column width) while
    // every other column — Date, Study, Break, Status — was left-aligned
    // at its own column start. That made the visual gap Break->Que
    // noticeably wider than the gap Que->Status (or the reverse,
    // depending on the text width), since only the Que column's text was
    // floating around a midpoint instead of starting flush at its slot
    // like its neighbors. Left-aligning Que the same as every other
    // column makes all four inter-column gaps a uniform colInnerGap.
