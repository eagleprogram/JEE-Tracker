...
For multi-device users, Firebase Firestore provides effortless, cross-device synchronization of study logs, planner tasks, sleep data, and syllabus progress. Reminders now have two delivery channels: in-tab notifications (always on, no setup) and an optional background push channel — powered by Firebase Cloud Messaging and a free GitHub Actions cron job — that reaches the device even when the browser is fully closed. Everything is designed to work offline-first, ensuring students can maintain their focus without worrying about network connectivity. This v2.0 isn't just a feature update; it's a refactoring milestone, structured into modular, single-responsibility files, making it significantly more maintainable, performant, and resilient for the years of preparation ahead. It's more than a tracker—it's a complete daily study companion built with a developer's precision and a student's needs in mind.
...
- **Background Push:** Firebase Cloud Messaging (FCM) + a Node script scheduled by GitHub Actions
...
└── js/
    ...
    ├── notifications.js      — idle nudge, break overrun, reminders, pings (in-tab delivery)
...
In-tab reminders work out of the box and need no configuration. For reminders that should arrive even when the browser is fully closed, this repo adds an optional background channel:

1. The browser registers a push token with Firebase Cloud Messaging and saves it to the user's Firestore document (`js/push-notifications.js`).
2. `server/send-scheduled-alarms.js` reads notification settings from Firestore, works out what's due, and sends the matching push message(s).
...
- **GitHub Pages** — Hosting (repo: `eagleprogram/JEE-Tracker`)
- **Firebase** — Auth + Firestore (free Spark plan) + Cloud Messaging for background push
- **GitHub Actions** — Free scheduled cron job for background push delivery
